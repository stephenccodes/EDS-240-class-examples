...
### Note 1: It's okay to cut the y-axis of line graphs
...
lyme_pop |> 
  ...
  ggplot(aes(x = year, y = cases_per100k, group = state)) +
  geom_line() +
  scale_y_continuous(limits = c(0, NA)) # Force y origin to 0
```
...
lyme_pop |> 
  ...
  geom_area() +
  scale_x_discrete(expand = c(0, NA)) + # Expand area to panel margins
  scale_y_continuous(expand = c(0, NA)) # Expand area to panel margins
```
