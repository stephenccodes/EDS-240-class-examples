...
mko_clean |> 
  mutate(month_name = factor(month_name, levels = month.name)) |>  # month.name is base R
...
mko_clean |> 
  mutate(month_name = factor(month_name, levels = month.name)) |> 
  filter(month_name %in% c("April", "June", "October")) %>% # Fewer groups
  ggplot(aes(x = Temp_bot, fill = month_name)) +
  geom_histogram(position = "identity", alpha = 0.5, binwidth = 1) + # Modify bin width
  scale_fill_manual(values = c("#2C5374", "#ADD8E6", "#8B3A3A")) # Adjust colors
...
mko_clean |> 
  filter(month_name %in% c("April", "June", "October")) |> # Fewer groups
  ggplot(aes(x = Temp_bot, fill = month_name)) +
  geom_density(alpha = 0.5, adjust = 0.5) + # Adjust the smoothness of the density line(band width)
...
* Skipped the last two sections for time *
...
ggplot(mko_clean, aes(x = Temp_bot, y = month_name, fill = after_stat(x))) + # tells ggplot to compute the x values (representing Temp_bot) after the statistical transformation (density estimation) and map those computed x values to the fill aesthetic. 
  geom_density_ridges_gradient(rel_min_height = 0.01, # sets the threshold for the relative height of density curves (low numbers best)
                               scale = 3) # controls the extent to which the different densities overlap+
  scale_fill_gradientn(colors = c("#2C5374","#849BB4", "#D9E7EC", "#EF8080", "#8B3A3A")) +
  scale_y_discrete(limits = rev(month.name)) # Group order
```
...
* Skipped last two sections for time *
...
ggplot(mko_clean, aes(x = month_name, y = Temp_bot)) +
  geom_violin() +
  geom_boxplot(width = 0.1, color = "gray", alpha = 0.5, oputlier.color = "red") +
  scale_x_discrete(limits = rev(month.name)) +
  coord_flip()
```
